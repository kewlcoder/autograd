...
def concat_and_multiply(weights, *args):
    ...
    # cat_state.shape = (45,41)
    print("cat_state.shape = ", cat_state.shape)
...
def build_dataset(filename, sequence_length, alphabet_size, max_lines=-1):
    ...
    with open(filename) as f:
        content = f.readlines()
    # len(content) - raw = 127
    # print("len(content) - raw = ", len(content))
    content = content[:max_lines]
    content = [line for line in content if len(line) > 2]   # Remove blank lines
    # len(content) - cleaned = 45
    # print("len(content) - cleaned = ", len(content))
